...
export default function ChatbotScreen() {
  ...
  const [messages, setMessages] = useState<ChatMessage[]>([
    {
      id: '1',
      type: 'bot',
      content: 'Hi! I\'m Lily, your virtual assistant. How can I help you today? Select a category or tap a question below.',
    },
  ]);
  ...
  const fetchData = async () => {
    try {
      setIsLoading(true);
      const [faqsRes, categoriesRes, ticketsRes] = await Promise.all([
        apiService.getFAQs(),
  ...
  const handleSendMessage = () => {
    ...
    const matchingFAQ = faqs.find(
      (faq) =>
        ...
        inputText.toLowerCase().includes(faq.question.toLowerCase().split(' ').slice(0, 3).join(' '))
    );
    ...
    if (matchingFAQ) {
      ...
      botMessage = {
        id: (Date.now() + 1).toString(),
        type: 'bot',
        content: 'I couldn\'t find an answer to your question. Would you like to submit a support ticket? Our team will get back to you within 24-48 hours.',
      };
    }
  ...
  const handleSubmitTicket = async () => {
    ...
    try {
      ...
      fetchData();
      Alert.alert('Success', 'Your support ticket has been submitted. We\'ll respond within 24-48 hours.');
    } catch (error) {
    ...
    <SafeAreaView style={styles.container} edges={['top']}>
      ...
      <View style={styles.header}>
        <View style={styles.headerLeft}>
          ...
          <View>
            <Text style={styles.headerTitle}>Lily Assistant</Text>
      ...
      <View style={styles.tabs}>
        ...
        >
          <Ionicons
            ...
            size={18}
            color={activeTab === 'chat' ? '#1E3A5F' : '#6B7280'}
          />
        ...
        >
          <Ionicons
            name="ticket"
            size={18}
            color={activeTab === 'tickets' ? '#1E3A5F' : '#6B7280'}
          />
        ...
        >
          ...
          >
            ...
            >
              ...
              <Text style={styles.supportCTAText}>Can't find your answer? Contact Support</Text>
            </TouchableOpacity>
                  ...
                  <View style={styles.ticketFooter}>
                    ...
                    {ticket.responses.length > 0 && (
                      <View style={styles.replyBadge}>
                        <Ionicons name="chatbubble" size={12} color="#1E3A5F" />
                        <Text style={styles.replyCount}>{ticket.responses.length}</Text>
                      </View>
                    )}
                  </View>
                </TouchableOpacity>
              ...
              {['general', 'billing', 'maintenance', 'complaint', 'other'].map((cat) => (
                <TouchableOpacity
                  ...
                  style={[
                    ...
                    newTicket.category === cat && styles.categorySelectChipActive,
                  ]}
                ...
                >
                  <Text
                    style={[
                      ...
                      newTicket.category === cat && styles.categorySelectTextActive,
                    ]}
                  >
        ...
        <View style={styles.modalOverlay}>
          <View style={[styles.modalContent, styles.ticketDetailModal]}>
            <View style={styles.modalHeader}>
              ...
              <TouchableOpacity onPress={() => setTicketDetailVisible(false)}>
                <Ionicons name="close" size={24} color="#1E3A5F" />
              </TouchableOpacity>
            </View>
            ...
            {selectedTicket && (
              <ScrollView style={styles.ticketDetailScroll} showsVerticalScrollIndicator={false}>
                ...
                <View style={styles.ticketDetailMessage}>
                  <Text style={styles.ticketDetailLabel}>Your Message:</Text>
                  <Text style={styles.ticketDetailText}>{selectedTicket.message}</Text>
                  <Text style={styles.ticketDetailDate}>
                    {format(new Date(selectedTicket.created_at), 'MMM dd, yyyy h:mm a')}
                  </Text>
                </View>
                    ...
                    {selectedTicket.responses.map((response, index) => (
                      <View key={index} style={styles.responseItem}>
                        <View style={styles.responseHeader}>
                          <Text style={styles.responderName}>{response.responder_name}</Text>
                          <Text style={styles.responseDate}>
                            {format(new Date(response.created_at), 'MMM dd, h:mm a')}
                          </Text>
                        </View>
                ...
                {selectedTicket.status !== 'closed' && (
                  <View style={styles.replySection}>
                    <TextInput
                      ...
                      multiline
                    />
                    <TouchableOpacity style={styles.replyButton} onPress={handleReplyToTicket}>
